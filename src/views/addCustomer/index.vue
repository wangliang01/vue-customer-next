  ...
  <div class="add-customer">
    ...
    <div v-if="step <= 1" class="section-step">
      <section class="section">
        ...
        <y-field v-model="form.name" label="客户名称" required placeholder="请输入客户名称" clearable></y-field>
        <y-field v-model="form.addressName" label="客户地址" required placeholder="请选择客户地址" is-link></y-field>
        <y-field v-model="form.address" label="详细地址" required placeholder="请输入具体地址" clearable></y-field>
      </section>
      <section class="section">
        <y-field v-model="form.customerName" label="联系人" required placeholder="请输入联系人姓名" clearable></y-field>
        <y-field v-model="form.phone" label="联系电话" required placeholder="请输入客户联系电话" type="tel" maxlength="11" clearable></y-field>
        <y-field label="固定电话" placeholder="请输入客户固定电话" type="number" clearable></y-field>
...
import { createCustomerForm } from '@/hooks/useCustomer'
export default {
  name: 'AddCustomer',
  components: {
  },
  ...
  setup() {
    ...
    const step = ref(0) // 0 : 初始状态,不显示步骤条, 1: 表示第一步, 2: 表示第二步
    const fileList = ref([])
